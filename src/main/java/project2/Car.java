...
car. Create 2 sub classes: Sedan and Truck. The Truck class has a field as weight
...
class Truck extends Car {
    ...
    public double calculateSalePrice() {
        if (weight > 2000) {
            return carPrice - ((carPrice/100))* 10; // 10% discount
        } else {
            return carPrice -((carPrice/100))* 20; // 20% discount
        }
    }
}
...
class Sedan extends Car {
    ...
    public double calculateSalePrice() {
        if (length > 20) {
            return carPrice - ((carPrice/100)*5); // 5% discount
        } else {
            return carPrice - ((carPrice/100)*10); // 10% discount
        }
    }
